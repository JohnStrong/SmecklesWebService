...
class ShoppingListItemModelSpec extends AnyWordSpec with Matchers {
  ...
  private val testItem = ShoppingListItem(name = "Milk", quantity = 2, currencyCode = "GBP", unitAmountMinor = 129L, lineAmountMinor = 258L)
  ...
  "ShoppingListItem JSON serialization" should {
    ...
    "deserialize from JSON correctly and compute line_amount_minor" in {
      val json = Json.obj("name" -> "Milk", "quantity" -> 2, "currency_code" -> "GBP", "unit_amount_minor" -> 129)

      json.as[ShoppingListItem] shouldBe testItem
    }

    "compute line_amount_minor as quantity * unit_amount_minor" in {
      val json = Json.obj("name" -> "Bananas", "quantity" -> 3, "currency_code" -> "USD", "unit_amount_minor" -> 500)
    ...
    "fail to deserialize when name is empty" in {
      val json = Json.obj("name" -> "", "quantity" -> 2, "currency_code" -> "GBP", "unit_amount_minor" -> 129)

      json.validate[ShoppingListItem] shouldBe a[JsError]
    }

    "fail to deserialize when quantity is missing" in {
      val json = Json.obj("name" -> "Milk", "currency_code" -> "GBP", "unit_amount_minor" -> 129)
    ...
    "fail to deserialize when unit_amount_minor is missing" in {
      val json = Json.obj("name" -> "Milk", "quantity" -> 2, "currency_code" -> "GBP")

      json.validate[ShoppingListItem] shouldBe a[JsError]
    }

    "fail to deserialize when quantity is zero" in {
      val json = Json.obj("name" -> "Milk", "quantity" -> 0, "currency_code" -> "GBP", "unit_amount_minor" -> 129)

      json.validate[ShoppingListItem] shouldBe a[JsError]
    }

    "fail to deserialize when quantity is negative" in {
      val json = Json.obj("name" -> "Milk", "quantity" -> -1, "currency_code" -> "GBP", "unit_amount_minor" -> 129)

      json.validate[ShoppingListItem] shouldBe a[JsError]
    }

    "fail to deserialize when currency_code is too short" in {
      val json = Json.obj("name" -> "Milk", "quantity" -> 2, "currency_code" -> "GB", "unit_amount_minor" -> 129)

      json.validate[ShoppingListItem] shouldBe a[JsError]
    }

    "fail to deserialize when currency_code is too long" in {
      val json = Json.obj("name" -> "Milk", "quantity" -> 2, "currency_code" -> "GBPP", "unit_amount_minor" -> 129)

      json.validate[ShoppingListItem] shouldBe a[JsError]
    }

    "fail to deserialize when unit_amount_minor is negative" in {
      val json = Json.obj("name" -> "Milk", "quantity" -> 2, "currency_code" -> "GBP", "unit_amount_minor" -> -1)

      json.validate[ShoppingListItem] shouldBe a[JsError]
    }

    "accept zero for unit_amount_minor (line_amount_minor becomes 0)" in {
      val json = Json.obj("name" -> "Free Sample", "quantity" -> 1, "currency_code" -> "GBP", "unit_amount_minor" -> 0)
